...
**밟으면 소모된다.** 플레이어가 이동을 시작할 때(`TryStartNextMove`) 다음 변의 양옆 칸 중 하나가 거미줄에 덮여 있으면 `TryGetTrapCell` 이 **가장 가까운** 거미줄의 중심 칸을 돌려주고(중심이 3~4칸 떨어진 거미줄은 범위가 겹칠 수 있어, 첫 일치를 쓰면 발밑이 아닌 먼 쪽이 사라진다), `StepOnTrap` 이 그 거미줄을 `Destroy` 하며 `RunTrapSlow` 코루틴을 시작한다. 코루틴은 `Player.speedMultiplier` 를 `trapSlowMultiplier` 로 낮추고 `trapSlowDelay` 뒤 1 로 되돌린다. 거미줄 하나는 한 번만 작동하고, 느려진 채 다른 거미줄을 밟으면 이전 코루틴을 끊고 시간을 새로 센다(깜빡임과 같은 패턴).

`speedMultiplier` 의 주인은 이 코루틴 하나다. 이동 시작마다 배율을 다시 계산하지 않는다 — 그러면 칸이 0.05 라 다음 변에서 바로 1 로 돌아가 버려 지속 시간이 무의미해진다. 대신 **사망하면 `HandlePlayerDeath` 가 코루틴을 끊고 1 로 되돌린다.** 밟은 거미줄은 이미 사라졌고 부활 지점은 거기서 멀어서, 그대로 두면 화면에 원인이 없는 채로 느리게 움직이기 때문이다.

밟지 않은 거미줄은 중심 칸이 확보될 때 `Capture` 직후와 클리어 때 `Destroy` 한다.

`CheckEnemyContact` 는 `Update` 에서 **이동 블록보다 먼저** 부른다. 뒤에 두면 변을 건너는 중인 프레임은 early return 에 걸려 판정을 건너뛰는데, 거미줄로 느려지면 한 변에 여러 프레임이 걸려 하필 가장 피하기 어려울 때 판정이 드물어진다.

메모리: 적·거미줄은 QixManager 의 자식으로 `Instantiate` 하므로 씬이 내려가면 함께 사라진다. 거미줄은 밟혔을 때와 확보 시 명시적으로 `Destroy`. 적 코드에는 코루틴이 없고 타이머는 모두 `Tick` 안의 float 다. 감속·무적·깜빡임 코루틴은 모두 `QixScene` 이 소유하며 핸들을 들고 있다가 교체·해제한다.
