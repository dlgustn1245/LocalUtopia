...
**밟으면 소모된다.** 플레이어가 이동을 시작할 때(`TryStartNextMove`) 다음 변의 양옆 칸 중 하나가 거미줄에 덮여 있으면 `TryGetTrapCell` 이 그 거미줄의 중심 칸을 돌려주고, `StepOnTrap` 이 그 거미줄을 `Destroy` 하며 `RunTrapSlow` 코루틴을 시작한다. 코루틴은 `Player.speedMultiplier` 를 `trapSlowMultiplier` 로 낮추고 `trapSlowDelay` 뒤 1 로 되돌린다. 거미줄 하나는 한 번만 작동하고, 느려진 채 다른 거미줄을 밟으면 이전 코루틴을 끊고 시간을 새로 센다(깜빡임과 같은 패턴).
